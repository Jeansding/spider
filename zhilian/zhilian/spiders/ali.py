...
def base_process(data):
    ...
    for i in range(10):
        data['item_property_list' + str(i)] = lbl.fit_transform(data['item_property_list'].map(lambda x: str(str(x).split(';')[i])
...
def sub(train, test, best_iter):
    ...
    sub=pd.read_csv(r"C:\Users\Mengcao.Quan\Desktop\ali\ad\round1_ijcai_18_test_b_20180418.txt", sep="\s+")
    sub=pd.merge(sub,sub1,on=['instance_id'],how='left')
    sub=sub.fillna(0)
    #sub[['instance_id', 'predicted_score']].to_csv('result/result0320.csv',index=None,sep=' ')
    sub[['instance_id', 'predicted_score']].to_csv('result0426.txt',sep=" ",index=False)

train = pd.read_csv(r"C:\Users\Mengcao.Quan\Desktop\ali\ad\round2_train.txt", sep="\s+")
test = pd.read_csv(r"C:\Users\Mengcao.Quan\Desktop\ali\ad\round2_ijcai_18_test_a_20180425.txt", sep="\s+")
#train = pd.read_csv(r"C:\Users\Mengcao.Quan\Desktop\ali\ad\round1_ijcai_18_train_20180301.txt", sep="\s+")
#test = pd.read_csv(r"C:\Users\Mengcao.Quan\Desktop\ali\ad\round1_ijcai_18_test_a_20180301.txt", sep="\s+")
