...
class PhantomJSMiddleware(object):
    ...
    def process_request(cls, request, spider):
        if not request.meta.has_key('NoPhantomJS'):
            print "Phantom 启动"
            #driver = webdriver.Chrome()
            driver = webdriver.PhantomJS(executable_path = "/Users/aprilkuo/anaconda2/bin/phantomjs",service_args=['--ignore-ssl-errors=true', '--ssl-protocol=any'])
            driver.get(request.url)
            # try:
            #     ele = driver.find_element_by_xpath('//div[@id="show-answer-hide"]/i')
            #     ele.click()
            # except:
            #     pass
...
class HttpProxyMiddleware(object):
    ...
    def inc_proxy_index(self):
        ...
        #print ("now using new proxy: %s" % self.proxyes[self.proxy_index]["proxy"])
